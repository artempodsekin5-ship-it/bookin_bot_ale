...
async def ask_name(target: Message | CallbackQuery, state: FSMContext, is_edit: bool = False):
    ...
    prompt = (
        "Пожалуйста, напиши имя или ник ещё раз:"
        if is_edit
        else "<b>1/4: Как тебя зовут?</b>\n\nНапиши имя или ник:"
    )
...
async def ask_phone(target: Message | CallbackQuery, state: FSMContext, is_edit: bool = False):
    ...
    prompt = (
        "Отправь контакт или напиши номер ещё раз:"
        if is_edit
...
async def ask_city(target: Message | CallbackQuery, state: FSMContext, is_edit: bool = False):
    ...
    prompt = (
        "Напиши город съёмки ещё раз:"
        if is_edit
        else "<b>3/4: В каком ты городе?</b>\n\nГде планируем снимать:"
    )
    if isinstance(target, CallbackQuery):
        await target.message.answer(prompt, parse_mode="HTML", reply_markup=get_cancel_keyboard())
        await target.answer()
...
async def ask_format(target: Message | CallbackQuery, state: FSMContext, is_edit: bool = False):
    ...
    prompt = (
        "Выбери или напиши формат съёмки ещё раз:"
        if is_edit
...
async def process_phone(message: Message, state: FSMContext):
    ...
    if message.contact:
        ...
        if len(digits_only) < 7:
            await message.answer(
                "Номер телефона слишком короткий. Пожалуйста, укажи корректный номер:",
...
async def process_city(message: Message, state: FSMContext):
    ...
    if len(city) < 2:
        await message.answer("Название города слишком короткое. Введи город ещё раз:")
        return
...
async def show_summary(target: Message | CallbackQuery, state: FSMContext):
    ...
    summary_text = (
        ...
        f"Связь: {html.escape(str(phone))}\n"
        f"Город: {html.escape(str(city))}\n"
        f"Формат: {html.escape(str(fmt))}\n\n"
...
async def final_send_callback(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    admin_message = (
        ...
        "━━━━━━━━━━━━━━━━━━\n"
        f"Имя клиента: {html.escape(str(name))}\n"
